"""
Distributed evaluation script for SegFormer on Apple-DMS test set.
...
def find_best_checkpoint(run_dir: str) -> str:
    ...
    # Check trainer_state.json inside the latest checkpoint for best_model_checkpoint
    if checkpoint_dirs:
    ...
    # Fallback - return original path (might be a Hub model ID)
    return run_dir
...
def main(args):
    ...
    try:
        ...
        if is_main_process():
            print(f"Loaded processor from: {checkpoint_path}")
    except OSError:
        # Processor not in checkpoint, try base model
        base_model = "nvidia/segformer-b5-finetuned-ade-640-640"
        if is_main_process():
            print(f"Processor not found in checkpoint, loading from: {base_model}")
    ...
    dataset = load_dataset("AllanK24/apple-dms-materials-v2", split="test")
    ...
    if dist.is_initialized():
        if is_main_process():
            ...
            sample_indices_tensor = torch.tensor(
                sample_indices_list + [0] * (num_vis_samples - len(sample_indices_list)),
                device=device
            )
        else:
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(iterator):
            ...
            # Process each sample - upsample logits to original size and get predictions
            logits = outputs.logits  # [B, C, H/4, W/4]
            
            for i, (label, image, orig_size) in enumerate(zip(labels, images, original_sizes)):
                # Upsample single sample's logits to original size
                sample_logits = logits[i:i+1]  # [1, C, H/4, W/4]
                ...
                pred_seg = upsampled_logits.argmax(dim=1).squeeze(0)  # [H, W]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate SegFormer on Apple-DMS test set")
    parser.add_argument(
        "--run_dir",
        type=str,
        required=True,
        help="Path to training run directory (best checkpoint will be selected automatically based on eval_mean_iou)",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Batch size per GPU for inference (default: 64)",
    )
    parser.add_argument(
        "--compile",
        action="store_true",
